...
@section('style')
            ...
            <div class="panel panel-default" data-sortable-id="table-basic-2">
                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-12 col-sm-12">
                            {{--<form action="{{ route('vocation.index') }}" class="form-inline pull-right">
                        ...
                        @if(count($lists))
                            @foreach($lists as $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="my-span" onclick="changeStatus('vocation', 'id', '{{ $item->id }}', 'status', this)">
                                            {!! $item::$statusText[$item->status] !!}
                                        </span>
                                    </td>
                                    <td>
                                        <a href="{{ route('vocation.edit', ['id'=>$item->id]) }}" data-toggle="modal"
                                           data-target="#modal-default-edit"
                                           class="btn btn-success btn-xs"><i class="fa fa-edit"></i> 编辑</a>
                                        <a href="javascript:void(0);"
                                           data-href="{{ route('vocation.destroy', ['id'=>$item->id]) }}"
    ...
    <div class="modal fade" id="modal-dialog">
        <div class="modal-dialog">
            <form class="form-horizontal" data-parsley-validate="true" name="demo-form"
                  action="{{ route('vocation.store') }}" method="post" id="create">
                {{ csrf_field() }}
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        <h4 class="modal-title">新增职业</h4>
                    </div>
    ...
    <div class="modal fade" id="modal-default-edit">
        <div class="modal-dialog">
            <form class="form-horizontal" action="{{ route('vocation.update') }}" id="edit" method="post"
